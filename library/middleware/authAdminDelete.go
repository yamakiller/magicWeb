...
//AuthAdminDelete Verify that you have delete
//Param  (*gin.Context) context
func AuthAdminDelete(context *gin.Context, db int, tokenSecret string, release bool) {
	if !release {
		context.Next()
		return
	}
	...
	if !auth.VerifyAdminProfile(profileItems.Items, context.Request.RequestURI, auth.ProfileDelete) {
		common.ResponseError(context, code.SpawnErrNeedPerm())
